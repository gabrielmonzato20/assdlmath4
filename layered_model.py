...
def define_dense_model_single_layer(input_length, activation_f='sigmoid', output_length=1):
    ...
    model = tf.keras.Sequential([
        ...
        tf.keras.layers.Dense(output_length, activation=activation_f)
    ])
...
def define_dense_model_with_hidden_layer(input_length, 
    ...
    model = tf.keras.Sequential([
        tf.keras.layers.Input(shape=(input_length,)),
        tf.keras.layers.Dense(hidden_layer_size, activation=activation_func_array[0]),
        tf.keras.layers.Dense(output_length, activation=activation_func_array[1])
    ])
